...
const MonthlyRanking: React.FC = () => {
  ...
  const chartOption = {
    ...
    yAxis: { type: 'category', data: data.slice(0, 10).map(d => d.storeName).reverse() },
    series: [{ type: 'bar', data: data.slice(0, 10).map(d => d.avgScoreRate).reverse(), itemStyle: { color: '#1890ff' } }],
  };
  ...
  const columns = [
    ...
    { title: '得分率(%)', dataIndex: 'avgScoreRate', key: 'avgScoreRate' },
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}><Card><Statistic title="平均得分率" value={data.length ? (data.reduce((s, d) => s + d.avgScoreRate, 0) / data.length).toFixed(1) : 0} suffix="%" /></Card></Col>
        <Col span={6}><Card><Statistic title="总检查次数" value={data.reduce((s, d) => s + d.inspectionCount, 0)} /></Card></Col>
        <Col span={6}><Card><Statistic title="合格率" value={data.length ? ((data.reduce((s, d) => s + d.passCount, 0) / Math.max(1, data.reduce((s, d) => s + d.inspectionCount, 0))) * 100).toFixed(1) : 0} suffix="%" /></Card></Col>
      </Row>
